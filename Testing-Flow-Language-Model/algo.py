...
class FLM(FLMBase):
    def training_step(self, batch, batch_idx):
        if not getattr(self.config.algo, 'prefix_training', False):
            return super().training_step(batch, batch_idx)
        ...
        # 1. Determine Loss Mask
        if 'loss_mask' in batch:
            loss_mask = batch['loss_mask']
        else:
            # Random Prefix Training Logic (for unconditional pre-training or standard P-FLM)
            # Sample random prefix lengths: [0, L-1]
            prefix_len = torch.randint(0, L, (B,), device=device)
            # mask[b, i] = 1 if i >= prefix_len[b] else 0 (Suffix indices)
            indices = torch.arange(L, device=device).abs().view(1, -1).expand(B, -1)
            loss_mask = (indices >= prefix_len.view(-1, 1)).float()
        
        current_accumulation_step = (
            batch_idx % self.trainer.accumulate_grad_batches)

        losses = self._loss(x0,
                            ...
                            loss_mask=loss_mask # Pass the mask
                            )
    ...
    def loss(self, x0, output_tokens,
             current_accumulation_step=None, train_mode=False,
             xT=None, given_t=None, not_sampling_t=False, loss_mask=None):
        ...
        # Apply loss mask if provided (e.g., for prefix conditioning)
    ...
    @torch.no_grad()
    def generate_with_prefix(self, prompt_ids, num_steps=None, eps=1e-5):
        """
        Generate samples conditioned on a prefix (Input Clamping).
        prompt_ids: (B, N) or (N,) Tensor
        """
        ...
        # 1. Create One-Hot prompt representation
        one_hot_prompt_short = F.one_hot(prompt_ids, num_classes=V).to(self.dtype).to(device)
        
        # 2. Sequence Construction [Prompt] [Noise]
        z = torch.randn((B, L, V), device=device, dtype=self.dtype)
        # Pad one_hot_prompt to size L
        one_hot_prompt = torch.zeros((B, L, V), device=device, dtype=self.dtype)
        prompt_len = min(N, L)
        one_hot_prompt[:, :prompt_len, :] = one_hot_prompt_short[:, :prompt_len, :]
        
        # Inject prompt into the starting noise
        z[:, :prompt_len, :] = one_hot_prompt[:, :prompt_len, :]
        
        # 3. Create Mask for Clamping
        mask = torch.zeros((B, L, 1), device=device, dtype=self.dtype)
        mask[:, :prompt_len, :] = 1.0
        ...
        for i in range(num_steps):
            ...
            # Predict x_1 (clean estimate)
            x_1_pred = self.forward(z, tau_t_in)
            x_1_pred_probs = x_1_pred.exp()
            
            if i == num_steps - 1:
                # Final step: hard clamp for the prompt, use prediction for the suffix
                z = (mask * one_hot_prompt) + ((1 - mask) * x_1_pred_probs)
                break
                
            # Velocity: v = (x_1 - z) / (1 - t)
            v = (x_1_pred_probs - z) / (1.0 - t_in.view(-1, 1, 1) + 1e-5)
            
            # Update sequence
            z_update = z + dt.view(-1, 1, 1) * v
            
            # 4. Input Clamping: Force prompt to remain constant
            z = (mask * one_hot_prompt) + ((1 - mask) * z_update)
            
        return z.argmax(dim=-1)

    @torch.no_grad()
    def generate_sliding_window(self,
                                ...
                                num_steps: int = None):
        """
        Generates sequences longer than the model's window size using sliding window sampling.
        """
        if window_size is None:
            window_size = self.config.model.length
        
        device = prompt_ids.device
        current_seq = prompt_ids.tolist()
        
        # 1. Initial generation
        generated = self.generate_with_prefix(
            prompt_ids=torch.tensor(current_seq, device=device),
            num_steps=num_steps
        )
        # Assuming generate_with_prefix returns [B, L]
        current_seq = generated[0].tolist()
        
        # 2. Iterative Sliding
        while len(current_seq) < max_total_length:
            # We take the tail of the current sequence as the new prompt
            new_prompt = current_seq[-overlap:]
            
            # Generate next chunk
            chunk = self.generate_with_prefix(
                prompt_ids=torch.tensor(new_prompt, device=device),
                num_steps=num_steps
            )
            
            # Append ONLY the new tokens (after the overlap)
            new_tokens = chunk[0].tolist()[overlap:]
            if not new_tokens:
                break
            
            current_seq.extend(new_tokens)
            
            # [SEP] token is usually 102 for BERT
            if 102 in new_tokens:
                break
                
        return torch.tensor([current_seq[:max_total_length]], device=device)
